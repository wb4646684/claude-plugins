...
async function login(account, password) {
  // 11位手机号自动补 +86
  if (/^1[3-9]\d{9}$/.test(account)) account = '+86' + account;
  ...
  if (!json?.data?.sessionId) {
    const code = json?.data?.accountResult;
    const [title, hint] = ACCOUNT_RESULT[code] ?? ['登录失败', null];
    const detail = hint ? `${title} — ${hint}` : `${title}: ${JSON.stringify(json).slice(0, 200)}`;
